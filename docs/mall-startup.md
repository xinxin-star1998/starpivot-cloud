...
商城已按 Guli 风格拆分为 **5 个业务微服务**，共用 Nacos 服务发现；各微服务使用独立业务库（见下表）。文件上传与商品图统一走 **阿里云 OSS**（`oss-config.yaml`）。
...
| `oss-config.yaml` | 阿里云 OSS（system、file、product、promotion 等共用，**商品图须启用**） |
...
1. Nacos「服务管理 → 服务列表」应出现 5 个 `starpivot-mall-*` 实例（healthy）。
...
```bash
mvn compile -pl starpivot-mall/starpivot-mall-member,starpivot-mall/starpivot-mall-product,starpivot-mall/starpivot-mall-ware,starpivot-mall/starpivot-mall-order,starpivot-mall/starpivot-mall-promotion -am
```
