<!-- 商城 SKU 列表（商品管理=只读检索；admin=完整维护） -->
<template>
  <div class="mall-sku-page art-full-height">
    ...
    <ElCard class="art-table-card" shadow="never" style="margin-top: 12px">
      <ArtTableHeader v-model:columns="columnChecks" :loading="loading" @refresh="refreshData">
        <template v-if="!isGoodsManager" #left>
          <ElButton
            v-if="hasAuth('mall:product:add')"
            type="primary"
...
const props = withDefaults(
  ...
  const isGoodsManager = computed(() => props.mode === 'goods-manager')
  ...
  const {
    ...
    core: {
      ...
      columnsFactory: () => buildDataColumns()
    },
    hooks: {
      onSuccess: (rows) => {
  ...
  const handleSearch = (params: Record<string, unknown>) => {
    Object.assign(searchParams, {
      ...
      spuId: parseOptionalNumber(params.spuId),
      catalogId: parseOptionalNumber(params.catalogId),
      brandId: parseOptionalNumber(params.brandId),
      minPrice: parseOptionalNumber(params.minPrice),
      maxPrice: parseOptionalNumber(params.maxPrice),
      spuPublishStatus: parseOptionalNumber(params.spuPublishStatus)
    })
    getData()
  }
</script>
