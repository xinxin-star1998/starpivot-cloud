...
<template>
  <div class="purchase-order-page art-full-height">
    ...
    <PurchaseDetailDrawer v-model:visible="detailVisible" :purchase-id="currentPurchaseId" />
    <PurchaseDoneDialog
...
<script setup lang="ts">
  ...
  function handleSelectionChange(selection: PurchaseVo[]) {
    selectedRows.value = selection.filter((r) => r.status === 0 || r.status === 1)
  }
  ...
  async function handleReceive() {
    ...
    if (!ids.length) {
      ElMessage.warning('请选择新建或已分配状态的采购单')
      return
    }
