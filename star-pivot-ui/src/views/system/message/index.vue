...
<template>
  <div class="message-center-page art-full-height">
    <MessageSearch v-model="searchForm" @search="handleSearch" @reset="handleResetSearch" />

    <ElCard class="art-table-card" shadow="never">
      <ArtTableHeader v-model:columns="columnChecks" :loading="loading" @refresh="refreshData">
        <template #left>
          <ElSpace wrap>
            <ElTag v-if="unreadCount > 0" effect="plain" type="danger"
              >未读 {{ unreadCount }}</ElTag
            >
            <ElButton
              v-auth="'system:message:send'"
              type="warning"
    ...
    core: {
      ...
      columnsFactory: () => [
        ...
        },
        {
          ...
          width: 90,
          formatter: (row: UserMessageVo) =>
            h(ElTag, { size: 'small', type: row.readFlag === '0' ? 'danger' : 'info' }, () =>
