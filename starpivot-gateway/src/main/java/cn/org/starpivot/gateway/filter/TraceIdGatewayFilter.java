...
public class TraceIdGatewayFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        // 使用 beforeCommit 在响应提交前写入 traceId，避免响应已提交后
        // headers 变为 ReadOnlyHttpHeaders 导致 UnsupportedOperationException
        exchange.getResponse().beforeCommit(() -> {
            ServerHttpResponse response = exchange.getResponse();
            if (!response.getHeaders().containsKey(TraceIdConstants.TRACE_ID_HEADER)) {
                response.getHeaders().set(TraceIdConstants.TRACE_ID_HEADER, traceId);
            }
            return Mono.empty();
        });
        syncTraceIdFromTracer(exchange);

        return chain.filter(exchange.mutate().request(mutatedRequest).build());
    }

    /**
     * 若 Tracer 可用，用当前 Span 的 traceId 覆盖响应头。
     * <p>
     * 使用 beforeCommit 回调在响应提交前写入，避免响应提交后
     * headers 变为 {@code ReadOnlyHttpHeaders} 导致 UnsupportedOperationException。
     *
     * @param exchange 当前请求的响应式上下文
     */
