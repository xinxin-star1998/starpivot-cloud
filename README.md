...
mysql -h127.0.0.1 -P3307 -uroot -proot star_pivot_member   < sql/star_pivot_member.sql
...
```
用户提交采购单
  → ware 更新 audit_status=PENDING
  → Feign 调 approval /internal/approval/instance/submit
  → 审批人待办 approve / reject（多步阶梯）
  → approval 事务提交后发 MQ（approval.instance.finished.mall.purchase）
  → ware 消费 starpivot.mall.approval-finished.purchase
...
| 数据库 | 已导入商城五域 SQL（至少 `star_pivot_ware.sql` 含采购单等业务表） |
| 微服务 | 至少启动 `gateway`、`auth`、`system`、`approval`，以及对应业务的商城服务（如采购需 `starpivot-mall-ware`） |
...
# 2. 重启相关微服务（auth / system / approval / 商城子服务 均 optional import mq-config.yaml）
mvn spring-boot:run -pl starpivot-system
mvn spring-boot:run -pl starpivot-auth
mvn spring-boot:run -pl starpivot-mall-ware
mvn spring-boot:run -pl starpivot-approval
mvn spring-boot:run -pl starpivot-gateway
```
...
| `starpivot.mall.approval-finished.purchase` | `approval.instance.finished.mall.purchase` | approval | ware |
| `starpivot.mall.approval-finished.spu` | `approval.instance.finished.mall.spu` | approval | product |
| `starpivot.mall.approval-finished.return` | `approval.instance.finished.mall.return` | approval | order |
| `starpivot.mall.approval-finished.coupon` | `approval.instance.finished.mall.coupon` | approval | promotion |
...
最后一关通过后，MQ 投递 `approval.instance.finished.mall.purchase`，ware 消费并更新业务表。
...
RabbitMQ **Queues** → `starpivot.mall.approval-finished.purchase`：最后一关通过后应有 Incoming / Ack。

ware 日志关键字：`采购单审批完结`；approval / mq 日志：`MQ 消息已发送`（routingKey=`approval.instance.finished.mall.purchase`）。

### 故障排查

| 现象 | 可能原因 | 处理 |
|------|----------|------|
| 看不到 `starpivot.*` 队列 | MQ 未启用或对应商城服务/system 未启动 | 确认 `MQ_ENABLED=true` 且已 re-import Nacos |
| 审批通过但 `audit_status` 仍为 PENDING | 对应商城服务未启 MQ 或消费失败 | 查 ware/product/order/promotion 日志、Redis 幂等、队列是否堆积 |
...
| 消息进入 DLX | 消费异常且重试耗尽 | 查对应商城服务错误日志与 DLX 队列 Payload |
...
Docker 中间件启动与故障排查详见 [docs/docker-deploy.md](docs/docker-deploy.md)。
