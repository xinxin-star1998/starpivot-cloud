...
public class WmsWareSkuServiceImpl extends ServiceImpl<WmsWareSkuMapper, WmsWareSku> implements WmsWareSkuService
{
    ...
    public void addStock(Long skuId, Long wareId, Integer skuNum) {
        ...
        if (existing == null) {
            ...
            SkuDto skuInfo = productFeignSupport.requireSku(skuId);
    ...
    public void inboundStock(WmsWareSkuInboundDTO dto) {
        productFeignSupport.requireSku(dto.getSkuId());
    ...
    private void enrichVoList(List<WmsWareSkuVO> voList) {
        ...
        Map<Long, Integer> warningMap = skuIds.isEmpty()
                ? Collections.emptyMap()
                : productFeignSupport.requireSkuMap(new ArrayList<>(skuIds)).entrySet().stream()
                        .collect(Collectors.toMap(Map.Entry::getKey, e -> e.getValue().getStockWarning() == null ? 0 : e.getValue().getStockWarning(), (a, b) -> a));
