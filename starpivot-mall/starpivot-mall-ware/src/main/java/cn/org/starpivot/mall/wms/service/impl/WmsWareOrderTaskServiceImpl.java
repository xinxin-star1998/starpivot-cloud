...
public class WmsWareOrderTaskServiceImpl extends ServiceImpl<WmsWareOrderTaskMapper, WmsWareOrderTask>
    ...
    public Long createFromOrder(Long orderId) {
        ...
        OrderInternalDto order = orderFeignSupport.requireOrderSummary(orderId);
        if (Integer.valueOf(1).equals(order.getDeleteStatus())) {
    ...
    public Long createFinishedRecordForPaidOrder(Long orderId, List<StockDeductionLineDto> deductions) {
        ...
        if (!CollectionUtils.isEmpty(deductions)) {
            for (StockDeductionLineDto line : deductions) {
    ...
    private WmsWareOrderTask buildTaskFromOrder(OrderInternalDto order) {
    ...
    private Long resolveTaskWareId(List<StockDeductionLineDto> deductions) {
        if (CollectionUtils.isEmpty(deductions)) {
            return null;
        }
        Set<Long> wareIds = deductions.stream()
                .map(StockDeductionLineDto::getWareId)
                .filter(id -> id != null)
                .collect(Collectors.toCollection(HashSet::new));
        return wareIds.size() == 1 ? wareIds.iterator().next() : null;
    }

    private void insertFinishedDetail(Long taskId, StockDeductionLineDto line) {
