...
public class WmsPurchaseServiceImpl extends ServiceImpl<WmsPurchaseMapper, WmsPurchase> implements WmsPurchaseService {
    ...
    public void addDetail(PurchaseDetailSaveBo bo) {
        SkuDto sku = productFeignSupport.requireSku(bo.getSkuId());
    ...
    private Map<Long, String> loadSkuNameMap(List<WmsPurchaseDetail> records) {
        ...
        return productFeignSupport.requireSkuMap(skuIds).entrySet().stream()
                .collect(Collectors.toMap(Map.Entry::getKey, e -> resolveSkuDisplayName(e.getValue()), (a, b) -> a));
    }

    private String resolveSkuDisplayName(SkuDto sku) {
        if (sku == null) {
            return null;
        }
