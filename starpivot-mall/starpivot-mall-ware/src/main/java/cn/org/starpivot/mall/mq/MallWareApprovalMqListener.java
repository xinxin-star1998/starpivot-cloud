...
public class MallWareApprovalMqListener extends AbstractMqListener<ApprovalFinishedMessage> {

    private final WmsPurchaseApprovalService purchaseApprovalService;

    public MallWareApprovalMqListener(MqMessageConverter messageConverter,
    ...
    @RabbitListener(
            queues = MqQueueNames.MALL_APPROVAL_FINISHED_PURCHASE,
            ackMode = "MANUAL",
    ...
    protected String resolveIdempotentKey(Message message, MessageEnvelope<ApprovalFinishedMessage> envelope) {
        ...
        if (payload != null && payload.getInstanceId() != null && payload.getResult() != null) {
            return "approval:finished:ware:" + payload.getInstanceId() + ":" + payload.getResult();
        }
