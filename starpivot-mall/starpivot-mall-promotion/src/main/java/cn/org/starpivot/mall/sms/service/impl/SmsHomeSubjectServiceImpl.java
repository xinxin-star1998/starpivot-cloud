 ...
 * 首页专题服务实现类。
 * <p>
 * 实现 {@link SmsHomeSubjectService}，处理首页专题相关业务。
 * </p>
 * <ul>
...
public class SmsHomeSubjectServiceImpl implements SmsHomeSubjectService {
    ...
    private List<HomeSubjectSpuVo> loadSpuList(Long subjectId) {
        ...
        if (!CollectionUtils.isEmpty(spuIds)) {
            spuNameMap = productFeignSupport.requireSpuMap(spuIds).entrySet().stream()
                    .collect(Collectors.toMap(Map.Entry::getKey, e -> e.getValue().getSpuName(), (a, b) -> a));
        }
