...
import cn.org.starpivot.api.product.dto.CategoryDto;
...
public class SmsHomeCategoryHotServiceImpl implements SmsHomeCategoryHotService {
    ...
    public List<HomeCategoryHotVo> listActive() {
        ...
        Set<Long> catIds = list.stream().map(SmsHomeCategoryHot::getCatId).collect(Collectors.toSet());
        Map<Long, String> catNameMap = loadCategoryNames(catIds);
        Map<Long, CategoryDto> categoryMap = loadCategories(catIds);
        return list.stream()
    ...
    private boolean isCategoryVisible(CategoryDto category) {
        if (category == null) {
            return false;
        }
    ...
    private void validateCategory(Long catId) {
        ...
        productFeignSupport.requireCategory(catId);
    }
    ...
    private Map<Long, String> loadCategoryNames(Set<Long> catIds) {
        ...
        return productFeignSupport.requireCategoryMap(catIds.stream().filter(id -> id != null).toList())
                .entrySet().stream()
                .collect(Collectors.toMap(Map.Entry::getKey, e -> e.getValue().getName(), (a, b) -> a));
    }

    private Map<Long, CategoryDto> loadCategories(Set<Long> catIds) {
        if (CollectionUtils.isEmpty(catIds)) {
            return Collections.emptyMap();
        }
        return productFeignSupport.requireCategoryMap(catIds.stream().filter(id -> id != null).toList());
    ...
    private HomeCategoryHotVo enrichPortalVo(
            SmsHomeCategoryHot entity, Map<Long, String> catNameMap, Map<Long, CategoryDto> categoryMap) {
        HomeCategoryHotVo vo = toVo(entity, catNameMap);
        CategoryDto category = entity.getCatId() != null ? categoryMap.get(entity.getCatId()) : null;
