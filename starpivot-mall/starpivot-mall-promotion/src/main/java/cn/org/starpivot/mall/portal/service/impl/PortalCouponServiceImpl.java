...
import cn.org.starpivot.api.mall.cart.CartInternalClient;
...
public class PortalCouponServiceImpl implements PortalCouponService {
    ...
    private OrderScope resolveOrderScope(Long memberId, PortalCouponTrialBo trialBo) {
        ...
        Set<Long> spuIdSet = skuMap.values().stream()
                .map(SkuDto::getSpuId)
                .filter(Objects::nonNull)
                .collect(Collectors.toSet());
        Map<Long, SpuDto> spuMap = spuIdSet.isEmpty()
                ? Map.of()
                : productFeignSupport.requireSpuMap(spuIdSet.stream().toList());
    ...
    private List<PortalOrderItemBo> resolveItems(Long memberId, PortalCouponTrialBo trialBo) {
        if (Boolean.TRUE.equals(trialBo.getUseCart())) {
            Result<List<CouponTrialItemDto>> result = cartInternalClient.listCheckedCartItems(memberId);
