...
public class PortalOrderServiceImpl implements PortalOrderService {
    ...
    public Map<String, Integer> statusCounts(Long memberId) {
        ...
        counts.put("review", productFeignSupport.countPendingReviews(memberId));
        return counts;
    }
    ...
    public PortalOrderSubmitVo submit(Long memberId, PortalOrderSubmitBo bo) {
        ...
        MemberAddressDto address = requireMemberAddress(memberId, bo.getAddressId());
        ...
        for (int i = 0; i < orderItems.size(); i++) {
            ...
            BigDecimal unitPrice = priced != null ? priced.getUnitPrice() : base.snapshot().getPrice();
        ...
        try {
            ...
            promotionFeignSupport.lockCoupon(memberId, bo.getCouponHistoryId(), order.getId(), orderSn);
            ...
            if (createdOrderId != null) {
                promotionFeignSupport.releaseCoupon(createdOrderId);
            }
            throw ex;
        }
    }
    ...
    public void cancel(Long memberId, Long orderId) {
        ...
        promotionFeignSupport.releaseSeckillByOrderSn(order.getOrderSn());
        promotionFeignSupport.releaseCoupon(orderId);
    ...
    private void enrichTrialLineTitles(
        ...
        for (PortalOrderPriceLineVo line : vo.getLines()) {
            ...
            String title = StringUtils.hasText(base.snapshot().getSkuTitle())
                    ? base.snapshot().getSkuTitle()
                    : base.snapshot().getSkuName();
    ...
    private MemberAddressDto requireMemberAddress(Long memberId, Long addressId) {
        return memberFeignSupport.requireAddress(memberId, addressId);
    }
