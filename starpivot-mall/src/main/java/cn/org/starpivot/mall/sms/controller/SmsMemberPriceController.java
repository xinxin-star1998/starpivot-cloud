 ...
 *   <li>{@link RequestMapping} — 基础路径 {@code /mall/member-price}</li>
 *   <li>{@link RequiredArgsConstructor} — 构造器注入服务依赖</li>
 *   <li>{@link Validated} — 启用方法级参数校验</li>
 *   <li>{@link Tag} — OpenAPI 分组「商城-会员价格」</li>
 * </ul>
 *
...
@RestController
@RequestMapping("/mall/member-price")
...
public class SmsMemberPriceController {
     ...
     * @return 业务数据
     */
    @Operation(summary = "会员价格详情")
    @GetMapping("/{id}")
    @PreAuthorize("hasAuthority('mall:memberprice:query')")
    public Result<MemberPriceVo> getById(@PathVariable("id") Long id) {
     ...
     * @return 操作结果
     */
    @Log(title = "新增会员价格", businessType = BusinessType.INSERT)
    @Operation(summary = "新增会员价格")
    @PostMapping
    @PreAuthorize("hasAuthority('mall:memberprice:add')")
    public Result<?> add(@Valid @RequestBody MemberPriceSaveBo bo) {
        smsMemberPriceService.add(bo);
     ...
     * @return 操作结果
     */
    @Log(title = "修改会员价格", businessType = BusinessType.UPDATE)
    @Operation(summary = "修改会员价格")
    @PutMapping
    @PreAuthorize("hasAuthority('mall:memberprice:edit')")
    public Result<?> update(@Valid @RequestBody MemberPriceSaveBo bo) {
        smsMemberPriceService.update(bo);
